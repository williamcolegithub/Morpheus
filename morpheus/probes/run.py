...
def layer1(model: str, seed: int) -> pd.DataFrame:
    ...
    print(f"  layer1: kept {len(df):,} cells across {df['cell_type'].nunique()} types (min={CELL_TYPE_MIN_CELLS})")
...
def layer2(model: str, seed: int, permute: bool) -> pd.DataFrame:
    ...
    print("computing gene-expression bins for matched negatives ...")
    adata = ad.read_h5ad(CELLS_H5AD, backed="r")
    ...
    candidate_bins = np.array([bins_map[g] for g in candidate_genes])
    ...
    for tf, targets in tqdm(tf_groups.items(), desc="layer2"):
        ...
        if permute:
            # Hewitt–Liang: permute labels — pick same-size random "target" set from candidate genes (excluding tf itself).
            mask = candidate_genes != tf
            pos_genes = list(rng.choice(candidate_genes[mask], size=len(pos_genes), replace=False))

        # Negative sampling: matched expression bins.
        ...
        # Single-fold AUC via 80/20 split on genes (no cell-fold here — gene-level probe).
        n = len(y)
        ...
        clf.fit(Xg[tr], y[tr])
        scores = clf.decision_function(Xg[te])
...
def layer3(model: str, seed: int) -> pd.DataFrame:
    """Hub identity probe + RSA between gene embeddings and DoRothEA TF-TF adjacency."""
    G, gene_idx, _ = _load_gene_embeddings(model)
    g2row = _gene_row_lookup(gene_idx)
    edges = pd.read_parquet(DOROTHEA_EDGES)
    edges = edges[edges["tf_ensembl"].isin(g2row) & edges["target_ensembl"].isin(g2row)].copy()
    ...
    print(f"  layer3: {len(tfs)} TFs; {is_hub.sum()} hubs (out-degree ≥ {threshold:.0f}, top {int(HUB_TOP_DECILE*100)}%)")
...
def main() -> None:
    ...
    for probe in probes:
        if probe == "layer1":
            df = layer1(args.model, args.seed)
        elif probe == "layer2":
            df = layer2(args.model, args.seed, args.permute_labels)
        elif probe == "layer3":
            df = layer3(args.model, args.seed)
        elif probe == "rsa":
            df = rsa(args.model, args.seed)
        else:
            raise ValueError(probe)
        if probe == "rsa":
            out = RESULTS / "rsa" / f"rsa__{args.model}{'__permuted' if args.permute_labels else ''}.parquet"
        else:
