...
\usepackage{tmlr}
% For camera-ready, swap to: \usepackage[accepted]{tmlr}
% For non-anonymous preprint posting, swap to: \usepackage[preprint]{tmlr}
...
\title{What scale buys for single-cell foundation models:\\
a controlled probe of Geneformer V1, V2-104M, and V2-316M}

% Author block is suppressed by the tmlr package in submission mode.
% It is preserved here for the [accepted] and [preprint] options.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig4.pdf}
...
Code, environment, and data-pull scripts are committed in an anonymized public repository (URL withheld for double-blind review; the repository is publicly accessible and will be linked in the camera-ready version) and archived at a long-term DOI (Zenodo, also anonymized for review). Pinned \texttt{uv.lock};
...
The full reproducible pipeline --- data acquisition, embedding extraction, probe runners, sensitivity sweep, statistics, and figure generation --- is released under the MIT License at a public repository (URL withheld for double-blind review) and archived at a long-term DOI (also withheld for review). All per-fold and per-TF results, the stats summary, the bibliography, and the figure-generation source are included; the camera-ready version will restore the repository URL and DOI.

\section*{Acknowledgments}

\textit{Acknowledgments are withheld for double-blind review and will be restored in the camera-ready version.}

\paragraph{AI assistance.} The author(s) used large language model assistants for code drafting, manuscript editing, and adversarial review during preparation. All scientific design, experimental execution, statistical analysis, and final interpretive judgments are the author(s)'; the author(s) take full responsibility for the manuscript's scientific content.
...
\textit{Funding statement withheld for double-blind review.}
