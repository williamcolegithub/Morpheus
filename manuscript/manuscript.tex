...
\title{What scale buys for single-cell foundation models: a controlled probe of Geneformer V1, V2-104M, and V2-316M}
\author{William Cole \\ Independent Researcher}
\date{2026-05-05}
...
\textbf{Status:} Draft v3 (2026-05-05). Single-author (Independent
Researcher). Targeting bioRxiv → eLife / TMLR / PLOS Comp Bio.

\subsection{Abstract}\label{abstract}

We probe whether Geneformer's frozen single-cell representations encode
...
identity (L1), TF→target structure (L2, expression-matched negatives),
and hub-TF identity (L3, top-decile out-degree in DoRothEA ∪ CollecTRI).
...
baseline by +0.20 AUC at every scale (CI {[}+0.13, +0.29{]} across all
three scales) --- a gap that survives a random-initialization control at
...
geometric room rather than learned structure. (2) \textbf{TF→target
probe gains are large and selectivity-passing across V1→V2 (+0.054 AUC,
p=10⁻¹⁶, n=375 TFs) but do not improve under pure parameter scaling
within V2} (V2-316M − V2-104M Δ=−0.003 {[}−0.014, +0.008{]}, p=0.45).
The V1→V2 gain is therefore not separable from pretraining-corpus
expansion. (3) \textbf{Cell-type identity (L1) shows no measurable
improvement under pure parameter scaling} (V2-316M − V2-104M Δ=−0.007
{[}−0.034, +0.017{]}); V2-316M fails to exceed the bag-of-genes baseline
(Δ=+0.019 {[}−0.001, +0.043{]}, p=0.31; we report this as ``fails to
...
The empirical case for scFM scale is becoming more guarded. Boiarsky et
al.~(2025) benchmark six scFMs against simple baselines and find that
the relative ranking is task- and tissue-dependent, with raw expression
baselines competitive on several integration metrics. Kedzierska et
al.~(2025) report that zero-shot evaluation reveals ``reliability
...
models the analogous question is well posed: Kaplan et al.~(2020)
established that loss scales as a power-law in parameter count, dataset
size, and compute over many orders of magnitude, and Hoffmann et
al.~(2022) --- the Chinchilla paper --- showed that compute-optimal
...
V2-104M → V2-316M (3× parameters, fixed corpus, fixed vocabulary).
...
lineage in NLP: Alain \& Bengio (2016, preprint) introduced linear
classifier probes for intermediate-layer analysis; Conneau et al.~(2018)
made probing canonical for sentence embeddings; Hewitt \& \citet{hewitt2019}
...
TF→target gene-regulatory edges, and (iii) the identity of
...
the ``geometric room'' alternative. Second, we show that the large V1→V2
gain on TF→target structure (+0.054 AUC, p=10⁻¹⁶) is not separable from
the pretraining-corpus and vocabulary expansion that accompany the V1→V2
transition, and that the only clean parameter-scaling axis available to
us (V2-104M → V2-316M) shows no measurable gain. Third, we show that on
cell-type identity Geneformer's largest publicly available variant fails
to exceed the bag-of-genes baseline (Δ=+0.019, p=0.31; we report this as
...
scran pooling, residual transformations) on the basis of Ahlmann-Eltze
and \citet{ahlmanneltze2023normalization}, who benchmarked four classes of single-cell
...
collision), the regulon contained 73,697 directed TF→target edges across
1,199 unique TFs. Critically, we did not apply a model-vocabulary filter
...
\subsubsection{Donor-stratified
cross-validation}\label{donor-stratified-cross-validation}
...
This protocol follows the principle articulated by Zimmerman, Espeland,
and \citet{zimmerman2021pseudoreplication} that cells from a common donor are not independent
...
and test. Although Zimmerman et al.~proposed mixed-effects models for
...
Three changes co-occur across the V1→V2 transition: parameter count (10×
larger), pretraining-corpus size (\textasciitilde3.5× larger), and
vocabulary (gc30M → gc104M, with associated changes in tokenizer
median-expression statistics). The V1→V2 step is therefore confounded as
a parameter-scaling experiment. The only clean parameter-scaling axis
available across publicly released Geneformer checkpoints is V2-104M →
...
labelled explicitly as either ``confounded (V1→V2)'' or ``parameter-only
(within V2)''.

\subsubsection{Embedding extraction}\label{embedding-extraction}
...
pipeline streamed cells.h5ad in chunks of 1,000 cells, densifying only
...
embedding capacity rather than to genuine TF→target structure.

\subsubsection{Probes}\label{probes}

All primary probes used scikit-learn \texttt{LogisticRegression} with
\texttt{C\ =\ 1.0}, the conservative probe class recommended by Hewitt
and \citet{hewitt2019} and \citet{belinkov2022}. Sensitivity to probe capacity is
...
The Layer 2 probe (TF→target identity) trains, per transcription factor,
...
TFs with at least 30 in-vocabulary targets, leaving 375 of 1,199 TFs.
...
corresponded to ≥143 targets, identifying 120 of 1,194 TFs as hubs. We
...
\subsubsection{Sensitivity sweep}\label{sensitivity-sweep}
...
in parallel. For the L1 and L3 probes, pairs are CV folds (n = 5); for
...
folds is uniform random per Methods); the L2 (TF→target edges) probe is
...
(V1→V2-104M, V1→V2-316M, within-V2, V2-316M − bag-of-genes) below and
visualise them in Figures 1--4.

\subsubsection{3.1 Hub-TF identity is encoded already at the smallest
...
DoRothEA network from non-hub genes using the gene's own embedding. All
...
model beat its random-init twin by a wide margin: Δ = +0.30 {[}+0.23,
+0.36{]} at 256-d (V1), Δ = +0.18 {[}+0.12, +0.26{]} at 768-d (V2-104M),
and Δ = +0.26 {[}+0.23, +0.33{]} at 1152-d (V2-316M). All three
contrasts hit the n=5 Wilcoxon floor of p = 0.0625; given the consistent
direction across three independent scales, the joint probability of
three same-sign matched-dimension wins under a no-effect null is 1/64
(sign-test, two-sided p ≈ 0.016). Performance against the bag-of-genes
baseline was similarly stable across scales (V2-316M − bag-of-genes Δ = +0.20 {[}+0.13, +0.29{]}, p = 0.0625). Critically, scaling the corpus
and parameter count from V1 to V2-316M did \textbf{not} improve
hub-identity readout: V2-316M − V1 Δ = -0.020 {[}-0.044, +0.026{]}, p =
0.625, and V2-316M − V2-104M Δ = -0.011 {[}-0.026, +0.052{]}, p = 1.00
(Figure 2). Hub-TF identity, in other words, is fully captured at V1
scale and neither lost nor improved by 9× more parameters and 3× more
pretraining cells.

\subsubsection{3.2 L2 TF→target edge readout gains from V1→V2 do not
survive
...
The V1→V2-104M jump was large and tightly estimated: paired Δ = +0.054
{[}+0.041, +0.068{]}, Wilcoxon p = 2.4 × 10⁻¹⁶ (n = 375). V1→V2-316M was
almost identical in magnitude (Δ = +0.051 {[}+0.038, +0.066{]}, p = 3.2
× 10⁻¹⁵). However the within-V2 contrast was a clean null: V2-316M −
V2-104M Δ = -0.003 {[}-0.014, +0.008{]}, Wilcoxon p = 0.45 (n = 375).
The \textasciitilde3× jump in parameter count from 104M to 316M, holding
the pretraining corpus fixed, produced no detectable improvement on
TF→target readout. Both V2 variants did, however, beat bag-of-genes by a
wide margin (V2-316M − bag Δ = +0.167 {[}+0.151, +0.182{]}, p = 4 ×
10⁻⁵⁰; V2-104M − bag indistinguishable from V2-316M − bag), so the L2
effect is real --- it simply does not scale within V2 (Figure 2). The
most parsimonious reading is that the V1→V2 step buys most of its L2
gain from the corpus expansion (\textasciitilde30M → \textasciitilde104M
cells) and the gc30M→gc104M vocabulary change between Geneformer V1 and
V2, not from raw parameter count.

\subsubsection{3.3 L1 cell-type identity does not exceed bag-of-genes at
any
...
≥200-cell threshold, of 71 in the full corpus) from the cell embedding.
...
1A). Both V2 variants tied with bag-of-genes within fold-level
uncertainty: V2-316M − bag-of-genes paired Δ = +0.019 {[}-0.001,
...
5). The within-V2 contrast was likewise null: V2-316M − V2-104M Δ =
...
from 0.393 (layer 0) to 0.430 (layer 12); layer 9 was the strongest
individual layer at 0.430, and the final-layer readout we use elsewhere
tied for best. No middle layer crossed the bag-of-genes line. The L1
...
better than its input gene tokens for this 47-way immune-typing task on
this corpus.

\subsubsection{3.4 Representational-similarity is null at every
...
symmetric DoRothEA TF→target adjacency, restricted to in-vocab TF--TF
pairs (n = 712,221 pairs per matrix). All three trained variants
returned essentially zero RSA: V1 ρ = -0.028, V2-104M ρ = -0.004,
V2-316M ρ = +0.022. Random-init transformers were similar (-0.001 to
-0.003), as was bag-of-genes (ρ = +0.026). No scale produced an RSA
...
\subsubsection{3.5 Sensitivity analyses confirm the headline
pattern}\label{sensitivity-analyses-confirm-the-headline-pattern}

Three pre-registered sensitivity analyses were run on every probe×model
combination to stress-test the headline claims.

\textbf{Probe family.} Replacing the linear probe with a small MLP (one
hidden layer, width 256, dropout 0.2) reduced rather than improved every
L2 and L3 contrast: V1 L2 MLP median AUC was 0.587 (down from 0.698
linear); V2-316M L2 MLP 0.701 (down from 0.755); V2-316M L3 MLP 0.567
(down from 0.698). The drop is consistent with overfitting on small
...
V2-316M MLP 0.460, bag MLP 0.366, V1 MLP 0.355), so the L1 V2 ≈
bag-of-genes story does not flip under a more expressive probe.

\textbf{Negative-sampling scheme for L2.} Switching from degree-matched
negatives to random negatives raised every per-TF AUC (e.g.~V2-316M
median 0.781 vs 0.755 matched), as expected when the negatives become
easier. Crucially the V1→V2 ordering and the within-V2 null both held:
V2-316M − V1 paired Δ = +0.069 {[}+0.062, +0.088{]} under random
negatives vs +0.051 {[}+0.038, +0.066{]} under matched, while V2-316M −
V2-104M remained null under both schemes. The headline claim --- V1→V2
jump real, within-V2 jump absent --- is robust to negative-sampling
choice.
...
prior (304 TFs) both reproduced the V1→V2 effect (V2-316M − V1 Δ =
...
saturates there; TF→target edges gain from the V1→V2 corpus and
...
\subsubsection{4.1 What we can conclude}\label{what-we-can-conclude}

Restricting attention to the only clean parameter-scaling axis the
public Geneformer checkpoints offer --- V2-104M → V2-316M, a 3× increase
...
show a measurable improvement. Cell-type identity moves by Δ=−0.007
macro-F1 (CI {[}−0.034, +0.017{]}); TF→target structure by Δ=−0.003 AUC
(CI {[}−0.014, +0.008{]}, p=0.45 over n=375 TFs); hub-TF identity by
Δ=−0.011 AUC (CI {[}−0.028, +0.007{]}). The combined V1→V2 transition
...
parameter count, \textasciitilde3.5× corpus expansion, and a vocabulary
...
benefit measurably from 30× more parameters or \textasciitilde3.5× more
...
\subsubsection{4.2 What we cannot
...
does not generalize. We do not claim to have decomposed the V1→V2 gain
...
V1→V2 gain on L1 is real, if power-limited, and the L2 TF→target signal
...
0.063 and our 95\% bootstrap CI on the V2-104M → V2-316M difference
...
comparison (Δ=+0.019, p=0.31) is similarly a failure to reject the null
...
\subsubsection{4.3 Connection to prior
...
dimensionality, plus an explicit decomposition of the V1→V2 transition
into parameter and corpus components. The selectivity-passing positive
findings we do report (L3 hub identity at every scale, L2 TF→target at
...
\subsubsection{4.4 What the field should take
...
scaling within the V2 family --- 3× more parameters at fixed data ---
...
the 3× inference-cost difference accordingly. Practitioners considering
...
the Venkatesh et al.~(2026) result is the more relevant precedent.
...
of Geneformer for the L2 TF→target and L3 hub-identity probes --- the
Civale et al.~(2026) result that intermediate layers carry task-relevant
...
Genecorpus-30M) would let the field cleanly decompose the V1→V2 confound
...
https://github.com/williamcolegithub/Morpheus. Pinned \texttt{uv.lock};
deterministic seed \texttt{0} threaded through every random component.
Census release 2025-11-08; \texttt{decoupler} 2.1.6;
\texttt{transformers} 4.57; PyTorch 2.5.1+cu124 (cloud) / 2.11+mps
(laptop). Pipeline reproduces end-to-end on a 24 GB MacBook Air (V1)
...
\subsection{Outstanding tickets}\label{outstanding-tickets}

\begin{itemize}
\tightlist
\item
  \textbf{PROBE-26} internal review pass (after 2--3 day cooling).
\item
  \textbf{PROBE-28} repo polish + reproducible README.
\item
  \textbf{PROBE-29} Zenodo snapshot + DOI.
\item
  \textbf{PROBE-30} bioRxiv submission.
\end{itemize}
