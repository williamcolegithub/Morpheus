...
Researcher). Targeting bioRxiv $\to$ eLife / TMLR / PLOS Comp Bio.
...
collision), the regulon contained 73,697 directed TF$\to$target edges across
...
model beat its random-init twin by a wide margin: $\Delta$ = +0.30 {[}+0.23,
+0.36{]} at 256-d (V1), $\Delta$ = +0.18 {[}+0.12, +0.26{]} at 768-d (V2-104M),
and $\Delta$ = +0.26 {[}+0.23, +0.33{]} at 1152-d (V2-316M). All three
...
(sign-test, two-sided p $\approx$ 0.016). Performance against the bag-of-genes
baseline was similarly stable across scales (V2-316M $-$ bag-of-genes $\Delta$ = +0.20 {[}+0.13, +0.29{]}, p = 0.0625). Critically, scaling the corpus
and parameter count from V1 to V2-316M did \textbf{not} improve
hub-identity readout: V2-316M $-$ V1 $\Delta$ = -0.020 {[}-0.044, +0.026{]}, p =
0.625, and V2-316M $-$ V2-104M $\Delta$ = -0.011 {[}-0.026, +0.052{]}, p = 1.00
(Figure 2). Hub-TF identity, in other words, is fully captured at V1
scale and neither lost nor improved by 9$\times$ more parameters and 3$\times$ more
pretraining cells.

\subsubsection{3.2 L2 TF$\to$target edge readout gains from V1$\to$V2 do not
survive
...
V2-316M $\rho$ = +0.022. Random-init transformers were similar (-0.001 to
-0.003), as was bag-of-genes ($\rho$ = +0.026). No scale produced an RSA
...
easier. Crucially the V1$\to$V2 ordering and the within-V2 null both held:
V2-316M $-$ V1 paired $\Delta$ = +0.069 {[}+0.062, +0.088{]} under random
negatives vs +0.051 {[}+0.038, +0.066{]} under matched, while V2-316M $-$
V2-104M remained null under both schemes. The headline claim --- V1$\to$V2
...
benefit measurably from 30$\times$ more parameters or \textasciitilde3.5$\times$ more
