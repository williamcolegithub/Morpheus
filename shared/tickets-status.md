...
| 2026-04-30 | PROBE-9 | embed | done | Geneformer V1-10M cell embeddings extracted on 100k cells in 10,361 sec on MPS, batch=16, dynamic-truncated. PROBE-11 attention sample (141 cells × 6L × 4H × 256² float16) saved as variable-shape object array | data/embeddings/geneformer/{cells.npy,cell_index.parquet,attention_sample.npz,META.json} |
| 2026-04-30 | PROBE-17 | controls | done | Random-init Geneformer (same architecture, untrained weights) embeddings extracted; same script via `--weights random_init` | data/embeddings/geneformer_random_init/* |
| 2026-04-30 | PROBE-18 | controls | done | Bag-of-genes baseline: cells stored sparse (CSR, nnz=146M, 100k×22k vocab), gene embeddings via TruncatedSVD on transposed matrix → 256 dim (explained-variance 0.633) | morpheus/embed/bag_of_genes.py, data/embeddings/bag_of_genes/* |
| 2026-04-30 | PROBE-13 | probe | done | Layer 1 cell-type identity (donor-stratified 5-fold, 39 types ≥200 cells, 98,434 cells): geneformer macro-F1=0.354, bag_of_genes=**0.404** (beats geneformer), random_init=0.219 | results/probes/layer1__*.parquet |
| 2026-04-30 | PROBE-14 | probe | done | Layer 2 TF→target probe with expression-matched negatives: geneformer median AUC=**0.694** (n=375 TFs), bag_of_genes=0.578, random_init=0.493 | results/probes/layer2__*.parquet |
| 2026-04-30 | PROBE-15 | probe | done | Layer 3 hub identity probe: geneformer median AUC=**0.718** (n=5 folds), bag_of_genes=0.499, random_init=0.425. RSA Spearman ρ=-0.028 (null) | results/probes/layer3__*.parquet, results/rsa/* |
| 2026-04-30 | PROBE-19 | controls | done | Hewitt–Liang selectivity (Layer 2 with permuted TF→target labels): AUC collapsed to 0.495 (median) — selectivity PASSES, probe is not overfitting | results/probes/layer2__geneformer__permuted.parquet |
| 2026-04-30 | PROBE-22 | writeup | done | Per-fold paired Wilcoxon + bootstrap CI on geneformer vs each baseline. Effect sizes reported alongside p-values. Layer 2 gf vs bag: +0.107 (p=10⁻³⁴); Layer 3 gf vs bag: +0.245 (p=0.06, n=5) | results/stats_summary.json, morpheus/analysis/stats.py |
| 2026-04-30 | PROBE-23 | writeup | done | Figures 1–4 generated as PDF + PNG: probe AUC bars + controls, per-TF AUC distribution, RSA, attention case study (mean per-layer attention erythroid vs other) | results/figures/fig{1,2,3,4}_*.pdf,png |
